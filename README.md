...
- Created Linked Service that connects to ADLS, call it as ls_adls

- Created Linked Service that connects to Azure SQL Database (ls_azuresqldb)

- Created Linked Service that connects to on-prem SQL server call it as SqlServer1

- created Linked Service that connects to AzureKeyVault, call it as ls_keyvault

- stored password for both ssms and AzureSqlDataBase in AzureKeyVault

Created dataset that connect to ADLS, call it as ds_adls

Created dataset that that use Azure SQL database call it as ds_azure_sql_db

Created dataset that use SQL Server management Studio (SSMS) call it ds_ssms.
...
copied the table from multiple databases in Sql server management studio to AzureDataLake and AzureSqlDB
